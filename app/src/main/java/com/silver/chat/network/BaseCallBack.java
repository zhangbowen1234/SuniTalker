...
//                Log.e("BaseCallBack", response.body().getStatusMsg() + "");
     ...
     * 后台返回的数据没有data调用这个
     */
    public static <T> void enqueueBase2(final Context context, Call<AddFriendBean> baseResponseCall, final ResponseCallBack<AddFriendBean> callBack) {
        baseResponseCall.enqueue(new Callback<AddFriendBean>() {
            @Override
            public void onResponse(Call<AddFriendBean> call, Response<AddFriendBean> response) {
                Log.e("BaseCallBack", response.body() + "");
                if (response.body().getCode() == 200 || response.body().getCode() == 1) {
                    callBack.onSuccess(response.body());
