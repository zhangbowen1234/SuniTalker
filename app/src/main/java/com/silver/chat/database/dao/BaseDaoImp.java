package com.silver.chat.database.dao;
...
import com.silver.chat.database.EasyDBConfig;
import com.silver.chat.database.callback.EasyRun;
import com.silver.chat.database.handler.HandlerHelper;
...
public class BaseDaoImp<T> implements BaseDao<T> {
    ...
    private PreparedQuery<T> fetchQueryBuilder(QueryBuilder<T, Integer> queryBuilder, WhereInfo whereInfo) throws SQLException {
        List<com.silver.chat.database.info.Where> wheres = whereInfo.wheres;
        if(!wheres.isEmpty()){
            Where<T, Integer> whereBuilder = queryBuilder.where();
            boolean isFirst = true;
            for(int i = 0; i < wheres.size(); i++){
                com.silver.chat.database.info.Where where = wheres.get(i);
                if(TextUtils.isEmpty(where.name))
                    continue;
                // 处理and-or
                isFirst = appendAnd(whereBuilder,where,isFirst);
                // 等于
                if(com.silver.chat.database.info.Where.EQ.equals(where.op)){
                    whereBuilder.eq(where.name, where.value);
                }
                // 模糊查询
                if(com.silver.chat.database.info.Where.LIKE.equals(where.op)){
                    whereBuilder.like(where.name,where.value);
                }
                // between
                if(com.silver.chat.database.info.Where.BETWEEN.equals(where.op)){
                    whereBuilder.between(where.name,where.low,where.high);
                }
                // lt 小于
                if(com.silver.chat.database.info.Where.LT.endsWith(where.op)){
                    whereBuilder.lt(where.name, where.value);
                }
                // gt 大于
                if(com.silver.chat.database.info.Where.GT.endsWith(where.op)){
                    whereBuilder.gt(where.name,where.value);
                }
                // ge 大于等于
                if(com.silver.chat.database.info.Where.GE.endsWith(where.op)){
                    whereBuilder.ge(where.name,where.value);
                }
                // le 小于等于
                if(com.silver.chat.database.info.Where.LE.endsWith(where.op)){
                    whereBuilder.le(where.name,where.value);
                }
                // ne 不等于
                if(com.silver.chat.database.info.Where.NE.endsWith(where.op)){
                    whereBuilder.ne(where.name,where.value);
                }
                // in 包含
                if(com.silver.chat.database.info.Where.IN.endsWith(where.op)){
                    whereBuilder.in(where.name,where.values);
                }
                // notIn 不包含
                if(com.silver.chat.database.info.Where.NOT_IN.endsWith(where.op)){
     ...
     * 追加连接符
     */
    private boolean appendAnd(Where<T, Integer> whereBuilder, com.silver.chat.database.info.Where where, boolean isFirst){
        if(!isFirst){
            if(com.silver.chat.database.info.Where.AND.equals(where.andOr)){
                whereBuilder.and();
            }
            if(com.silver.chat.database.info.Where.OR.endsWith(where.andOr)){
                whereBuilder.or();
            }
        }
        return false;
