...
public class RegisterPresenter extends BasePresenter<RegisterContract.View>
    ...
    public void register(String phone, String name, String password) {
        ...
        if (!checkMobile(phone)) {
            // 提示
            view.showError(com.example.lang.R.string.data_account_register_invalid_parameter_mobile);


        } else if (name.length() < 2) {
            // 姓名需要大于2位
            view.showError(com.example.lang.R.string.data_account_register_invalid_parameter_name);
        } else if (password.length() < 6) {
            // 密码需要大于6位
            view.showError(com.example.lang.R.string.data_account_register_invalid_parameter_password);
        } else {
            //进行网络请求
